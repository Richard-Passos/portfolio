...
const Lines = ({ className, style, ref, ...props }: LinesProps) => {
  ...
  return (
    <div
      className={cn(
        'pointer-events-none absolute inset-x-0 top-0 h-(--document-height) translate-y-(--document-top) bg-[linear-gradient(currentColor_.8px,transparent_.8px),linear-gradient(to_right,currentColor_.8px,transparent_.8px)] bg-size-[250px_200px] bg-center text-body-hover',
        className
      )}
