  ...
  return (
      <ScrollAnimate config={ANIMATION_CONFIG.x}>
        <Section
          hasTransition={false}
          className={cn(
            'flex py-0 w-9/10 max-w-screen-xl flex-col overflow-y-clip [--h:100vh] 2xl:[--h:--max-h]',
            className,
          )}
          {...props}
        >
...
const FooterContentCta = ({className, data = {}, ...props}) => {
  ...
  return <ScrollAnimate config={ANIMATION_CONFIG.y}>
  <div className={cn('relative z-10 py-md [--y:calc(var(--smooth-y)*var(--h))] sm:translate-y-[--y] dark:border-border/20', className)} {...props}>
    <Text className='text-muted-content first-letter:uppercase'>
      {data.subtitle}
    </Text>

    <Text.Title variants={{ size: 'xl' }} className='mt-1.5 whitespace-pre-line'>
      {data.title}
    </Text.Title>
    ...
    <span className='absolute inset-y-0 left-1/2 -z-10 w-screen -translate-x-1/2 border-b dark:box-border/20 border-border/60 bg-main shadow-[0_25px_35px_-15px] shadow-[hsl(var(--content)/.025)] dark:shadow-none' />
  </div>
</ScrollAnimate> 
}
...
const FooterContentPersonal = async ({className, data = {}, ...props}) => {
  ...
  return <ScrollAnimate config={ANIMATION_CONFIG.y}>
  <div className={cn('relative flex grow justify-between gap-md py-md [--y:calc(var(--smooth-y)*var(--h))] max-sm:flex-col sm:translate-y-[--y] sm:items-end', className)} {...props}>
    <div className='flex flex-col items-start'>
      ...
      <section className='flex mt-4 flex-col gap-1.5'>
    {data.smalls?.map((data, i) => {
      ...
      return <Text.Small key={i} className='text-xs'>
          {hasA ? A : data.label}
          </Text.Small>
    })}
      </section>
      ...
      <BackTop asChild >
        <Button
        className='text-xs mt-md max-sm:hidden font-bold uppercase'
        {...action.data}      
    >
    ...
    <GridPattern className='right-auto -top-px left-1/2 w-screen -translate-x-1/2' />
  </div>
</ScrollAnimate>
}
