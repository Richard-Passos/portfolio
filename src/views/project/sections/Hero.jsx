...
const ProjectViewHeroSection = ({
  ...
  return (
    <Section
      forceHeaderTheme
      className={cn(
        'z-20 -mt-[--header-h] !min-h-[calc(var(--h)*1.5)] overflow-y-clip py-0 [--h:100svh] 2xl:[--h:--max-h]',
        className,
      )}
      {...props}
    >
      <ScrollAnimate.Transform config={ANIMATION_CONFIG.y1}>
        <div className='relative flex min-h-[--h] flex-col items-center justify-between pb-md pt-[calc(theme(spacing.md)+var(--header-h))]'>
          <Text.Title
            asChild
            className='w-9/10 max-w-screen-lg text-center'
            variants={{ size: 'xl' }}
          >
            <h1>{project.title}</h1>
          </Text.Title>
          ...
          <section className='mt-md flex w-9/10 max-w-screen-xl items-center justify-between'>
            {data.actions?.map((action) => (
              ...
              >
                ...
                <span
                  className={cn(
                    action.type?.toLowerCase() === 'next' && '-order-1',
                  )}
                >
                  {action.data?.label}
                </span>
          ...
          <ScrollAnimate.Transform config={ANIMATION_CONFIG.y2}>
            <div className='pointer-events-none absolute inset-0 z-10 drop-shadow-[0_0_25px_hsl(var(--content)/.05)] dark:drop-shadow-none'>
