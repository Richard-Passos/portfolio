...
import { Button, ScrollAnimate, ScrollTitle, Section, Services, TextScrollAnimate } from '@/components';
...
const ContactViewServicesSection = async ({
  ...
  return (
    ...
    >
      ...
      <ContactViewServicesSectionBlock className='mt-md' data={data.block} />
    </Section>
  );
};

const ContactViewServicesSectionBlock = ({ className, data = {}, ...props }) => {
  return <div className={cn('grid w-9/10 max-w-screen-lg gap-md sm:grid-cols-2', className)} {...props} >
  ...
  {data.icons?.map((data, i) => (
    ...
    >
      <div className='absolute size-[min(30vmin,theme(maxWidth.xs))] first:bottom-0 first:left-0 last:top-0 last:right-0'>
        <Icon
